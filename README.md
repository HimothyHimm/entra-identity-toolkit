...
## Unattended authentication (service principal)

Early stages ran interactively via delegated sign-in. The toolkit now provisions
its own Microsoft Entra app registration and authenticates as a service principal
using a self-signed certificate — the way identity automation runs in production
(scheduled jobs, CI/CD, no human in the loop).

`New-ToolkitAppRegistration.ps1` is run once by an admin. It creates the app +
service principal, generates a signing certificate in the user cert store, uploads
the public key, and grants admin-consented Microsoft Graph application permissions
— all via raw Graph, idempotently.

| Permission | Used by |
|---|---|
| User.ReadWrite.All | onboarding |
| Group.ReadWrite.All | department groups |
| Organization.Read.All | license / SKU reads |
| Directory.Read.All | directory reporting |
| RoleManagement.ReadWrite.Directory | role assignment (cloud-admin provisioning) |

`RoleManagement.ReadWrite.Directory` is the highest-privilege grant; it is included
deliberately because the toolkit performs directory role assignment. In a production
tenant it would be justified or scoped down rather than granted by default.

A certificate is used rather than a client secret so no secret material lives in a
file — the private key stays in the Windows certificate store and is referenced by
thumbprint. `Connect-ToolkitGraph.ps1` is the single auth entry point: app-only when
a ClientId and thumbprint are supplied, interactive otherwise.